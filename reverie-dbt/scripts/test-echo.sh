...
expected="hello from reverie-dbt"
"$drrun" -quiet -disable_rseq -stack_size 2M -c "$client" -summary -- \
  /bin/echo "$expected" >"$tmpdir/stdout" 2>"$tmpdir/stderr"

[[ $(cat "$tmpdir/stdout") == "$expected" ]]
grep -Eq 'reverie-dbt: branches=[1-9][0-9]* syscalls=[1-9][0-9]* rewritten_writes=[1-9][0-9]*' \
  "$tmpdir/stderr"
cat "$tmpdir/stderr"

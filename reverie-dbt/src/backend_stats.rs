...
//! Typed end-of-run statistics owned by the DynamoRIO (DBT) backend.
//!
//! Unlike the in-process LiteInst provider, DBT accumulates its counters inside
...
//! record from the whole process tree into a single [`DbtBackendStatsSnapshot`].
//!
//! This module owns three things that make that possible without any text
//! parsing (the historical `reverie-dbt: tool=... branches=...` stderr line):
//!
//! * the typed schema ([`DbtBackendStatsSnapshot`], [`DbtTranslationStats`],
//!   [`DbtRuntimeKind`]) and its [`reverie::BackendStatsSource`] adapter;
//! * a stable, C-emittable wire ABI ([`encode_process_record`],
//!   [`decode_process_record`]) that the native client writes verbatim;
//! * an order-independent [`DbtBackendStatsAggregator`] that folds records from
...
/// Eight-byte magic prefixing every wire record: `RVDBTSTA` (Reverie DBT stats).
pub const WIRE_MAGIC: [u8; 8] = *b"RVDBTSTA";
...
/// Which built-in DBT client runtime produced a process record.
///
/// Mirrors `reverie_dbt_runtime_name` in the native client. `Ord` so a
/// [`CounterSnapshot`] over these keys renders in a deterministic order.
#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub enum DbtRuntimeKind {
...
impl DbtRuntimeKind {
    ...
    ///
    /// Any unrecognized code decodes to [`DbtRuntimeKind::Unknown`] rather than
...
pub struct DbtProcessRecord {
    ...
    /// Counted control-flow branches (the DBT branch clock).
    pub branches: u64,
...
impl Default for DbtProcessRecord {
    fn default() -> Self {
        Self {
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Truncated {
                ...
                formatter,
                "DBT stats record truncated: {available} of {required} bytes"
            ),
            Self::BadMagic => formatter.write_str("DBT stats record has an invalid magic prefix"),
            Self::UnsupportedVersion(version) => {
                write!(
                    formatter,
                    "DBT stats record has unsupported version {version}"
                )
            }
            Self::BadRecordLength(length) => {
                write!(
                    formatter,
                    "DBT stats record self-reports invalid length {length}"
                )
            }
        }
    }
...
pub fn encode_process_record(record: &DbtProcessRecord) -> [u8; WIRE_RECORD_LEN] {
...
pub fn decode_process_record(bytes: &[u8]) -> Result<(DbtProcessRecord, usize), WireError> {
    ...
    let runtime_kind = DbtRuntimeKind::from_wire(bytes[16]);
...
pub struct DbtTranslationStats {
...
/// Stable DBT statistics captured after one backend run over a whole process tree.
...
pub struct DbtBackendStatsSnapshot {
    ...
    translation: DbtTranslationStats,
    runtime_kinds: CounterSnapshot<DbtRuntimeKind>,
}

impl DbtBackendStatsSnapshot {
    ...
    pub const fn translation(&self) -> &DbtTranslationStats {
        &self.translation
    }

    /// Per-runtime-kind process-image counts in deterministic key order.
    pub const fn runtime_kinds(&self) -> &CounterSnapshot<DbtRuntimeKind> {
        &self.runtime_kinds
    }
}

impl fmt::Display for DbtBackendStatsSnapshot {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "DBT instrumentation stats: process_images={} branches={} syscalls={} rewritten={} stdin_reads={} memory_hash_fold={:016x} translation[basic_blocks_built={} threads_created={} synchronization_retries={} native_signals={} code_cache_exits={} peak_threads_per_process={} peak_reachable_code_cache_blocks_per_process={} images_with_stats={} images_without_stats={}] runtimes[",
            self.process_images,
            self.counted_branches,
            self.intercepted_syscalls,
            self.rewritten_syscalls,
...
pub struct DbtBackendStatsAggregator {
    ...
    process_images_with_stats: u64,
    process_images_without_stats: u64,
    runtime_kind_counts: Vec<(DbtRuntimeKind, u64)>,
}

impl DbtBackendStatsAggregator {
    ...
    pub fn into_source(self) -> DbtBackendStatsSource {
        DbtBackendStatsSource {
            snapshot: self.snapshot(),
        }
    }
}

/// Backend-owned source for a typed DBT end-of-run snapshot.
#[derive(Clone, Debug)]
pub struct DbtBackendStatsSource {
    snapshot: DbtBackendStatsSnapshot,
}

impl DbtBackendStatsSource {
    /// Wraps an already-aggregated snapshot as a source.
    pub const fn from_snapshot(snapshot: DbtBackendStatsSnapshot) -> Self {
        Self { snapshot }
    }

    /// Returns the captured snapshot without recomputing it.
    pub const fn snapshot(&self) -> &DbtBackendStatsSnapshot {
        &self.snapshot
    }
}

impl fmt::Display for DbtBackendStatsSource {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.snapshot.fmt(formatter)
    }
}

impl BackendStatsSource for DbtBackendStatsSource {
    type Snapshot = DbtBackendStatsSnapshot;
...
mod tests {
    ...
    fn sample_record(seed: u64, dr_stats: bool) -> DbtProcessRecord {
        DbtProcessRecord {
    ...
    fn unknown_runtime_code_decodes_to_unknown_not_error() {
        ...
        assert_eq!(decoded.runtime_kind, DbtRuntimeKind::Unknown);
    }
    ...
    fn aggregation_is_order_independent() {
        ...
        let mut forward = DbtBackendStatsAggregator::new();
        for record in records.iter() {
            forward.record(record);
        }

        let mut reverse = DbtBackendStatsAggregator::new();
    ...
    fn runtime_kind_counts_are_deterministic() {
        ...
        prototype.runtime_kind = DbtRuntimeKind::PrototypeTool;
        let mut counter1 = sample_record(1, true);
        counter1.runtime_kind = DbtRuntimeKind::Counter1;
        ...
        assert_eq!(
            ...
            &[
                (DbtRuntimeKind::PrototypeTool, 1),
                (DbtRuntimeKind::Counter1, 2)
            ]
        );
    }
    ...
    fn disabled_request_does_not_collect_snapshot() {
        let source = DbtBackendStatsAggregator::new().into_source();

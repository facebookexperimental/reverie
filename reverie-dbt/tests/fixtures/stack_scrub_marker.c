...
/*
 * Negative control for the DBT guest-stack scrub (`scrub_guest_stack_residue`
 * in `reverie-dbt/native/client.c`): the scrub must not erase the guest's own
 * dead stack.
 *
 ...
 * marker (90/90/90) while DBT zeroed it after a clone in 4 of 4 runs.
 *
 ...
 * Note it must not consult `/proc/self/maps` to find its stack -- the DBT

    ...
    let response = match GUEST_CLIENT.try_lock() {
            ...
            if must_connect {
                ...
                *slot = Some(
                    connect_guest(context, invoke_syscall, &path, pid)
                        .expect("reverie-dbi sync_rpc: failed to connect guest socket"),
                );
            }
            let connection = slot
                .as_ref()
                .expect("reverie-dbi sync_rpc: guest connection initialization failed");
        ...
        Err(TryLockError::Poisoned(poisoned)) => {
            ...
            let connection = connect_guest(context, invoke_syscall, &path, pid)
                .expect("reverie-dbi sync_rpc: failed to recover guest connection");
        ...
        Err(TryLockError::WouldBlock) => {
            ...
            let connection = connect_guest(context, invoke_syscall, &path, std::process::id())
                .expect("reverie-dbi sync_rpc: failed to connect contended guest socket");
            let response = guest_round_trip(context, invoke_syscall, connection.fd, &request_bytes);
            close_guest(context, invoke_syscall, connection.fd);
            response
        }
    }
    .expect("reverie-dbi sync_rpc: guest coordinator round trip failed");
    decode(&response)
}
...
fn unix_address(path: &Path) -> std::io::Result<libc::sockaddr_un> {
    ...
    if bytes.is_empty() || bytes.len() >= 108 {
        return Err(std::io::Error::new(
            ...
            "reverie-dbi sync_rpc: coordinator socket path is empty or too long",
        ));
    }
...
fn read_frame_from_guest(
    ...
    if len > MAX_FRAME_LEN {
        return Err(std::io::Error::new(
            ...
            format!("reverie-dbi sync_rpc: frame length {len} exceeds {MAX_FRAME_LEN}"),
        ));
    }

...
async fn main() -> io::Result<()> {
    ...
    let Some(program) = arguments.first().cloned() else {
        return Err(io::Error::new(
            ...
            "usage: reverie-dbi-counter2 -- PROGRAM [ARG ...]",
        ));
    };
    ...
    guest.args(guest_arguments).env("HERMIT_DBI_COUNTER2", "1");
    ...
    eprintln!(
        "reverie-dbi: counter2 global system calls: {syscalls}, from {processes} processes, {threads} thread(s)"
    );

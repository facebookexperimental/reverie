...
/*
 * Regression fixture for the DBI `Guest::backtrace` path. It issues a getpid
 ...
 * HERMIT_DBI_TEST_BACKTRACE tool intercepts the getpid, captures the guest
 * backtrace (seeded from the guest register file and walked over the guest's own
 * stack, since the DBI Tool runs in the guest address space), and emits a line
...
// TODO-HUMAN-REVIEW(PR-ratchet16): Review the DBI backtrace probe fixture.

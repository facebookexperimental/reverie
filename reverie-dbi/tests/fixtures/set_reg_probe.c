...
/*
 * Regression fixture for the DBI `Guest::set_regs` path. It loads a known value
 * into the callee-saved r15 register, issues a getpid syscall (which the
 * HERMIT_DBI_TEST_SET_REG tool intercepts to overwrite r15 via set_regs), then
...
// TODO-HUMAN-REVIEW(PR-167): Review the DBI set_regs probe fixture.
int main(void) {
  uint64_t observed = 0;
  __asm__ volatile(

...
/*
 * Regression fixture for the DBI `Guest::ppid` path (and the `is_root_process`
 * default built on it). It issues a getpid syscall, which the
 * HERMIT_DBI_TEST_PPID tool intercepts to emit a line reporting the tool-observed
 ...
 * -> `reverie_dbi_runtime_thread_init` -> `PROCESS_PPID` -> `current_ppid` ->
 * `Guest::ppid`. The non-root case (a real in-tree parent) is covered by the
 * `reverie-dbi` unit tests, which drive `current_ppid`/`Guest::ppid` with a
...
// TODO-HUMAN-REVIEW(PR-ratchet11): Review the DBI ppid probe fixture.
int main(void) {
  syscall(SYS_getpid);
  return 0;
}

...
extern void reverie_dbi_runtime_exec_failed(
...
extern void reverie_dbi_runtime_totals(
    uint64_t* branches,
    uint64_t* syscalls,
    uint64_t* rewritten,
...
static uint64_t observe_virtual_time(void) {
  return atomic_fetch_add_explicit(
      &virtual_time_ns, UINT64_C(1000), memory_order_seq_cst);
}
...
static void advance_virtual_time(uint64_t nanoseconds, bool absolute) {
  if (!absolute) {
    atomic_fetch_add_explicit(
        &virtual_time_ns, nanoseconds, memory_order_seq_cst);
    return;
  }
...
DR_EXPORT void dr_client_main(client_id_t id, int argc, const char* argv[]) {
  ...
  if (!dr_create_client_thread(
          runtime_background_init, (void*)reverie_dbi_emit))
    DR_ASSERT(false);
  ...
  if (!drmgr_register_module_load_event(module_load) ||
      ...
      !drmgr_register_bb_instrumentation_event(
          NULL, instrument_instruction, NULL) ||

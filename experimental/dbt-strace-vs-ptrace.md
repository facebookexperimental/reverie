# DBT strace tool vs ptrace strace — comparison (DBT M4 validation)

Task `impl-dbt-m4-strace-tool`, 2026-07-23. Validates the DBT Guest/Tool
interface by running a strace-like `reverie::Tool` over the DynamoRIO-based
`DbtGuest` and comparing its output to the ptrace-based strace example on the
same binary.

## What already existed (not net-new)

`reverie-dbt/src/tools.rs` already contains two observation tools built on the
...
- **`SyscallCounterTool`** — per-number histogram at exit. Enabled with
  `HERMIT_DBT_SYSCALL_HISTOGRAM=1`.

They share reverie's `syscalls::Displayable`, so decoding matches the ptrace
tool. Dispatch is via `run_active_tool()` in the native client; a `DbtGuest` is
...
PROFILE=release reverie-dbt/scripts/build-client.sh
CLIENT=target/release/reverie-dbt-native/libreverie_dbt_client.so
DRRUN=$(target/release/reverie-dbt-dynamorio-path drrun)

# DBT strace
HERMIT_DBT_STRACE=1 "$DRRUN" -disable_rseq -c "$CLIENT" -- /bin/echo hello
...
| metric | DBT StraceTool | ptrace strace |
|--------|----------------|---------------|
| syscalls traced | 113 | 114 (incl. `execve`) |
| syscalls after removing `execve` | 113 | 113 |
| syscall-name sequence match | **111/113 identical** | — |
| stdout preserved | yes (`hello-dbt`) | yes |
| run-to-run stability (DBT) | stable, 113 identical ×N | — |
| decode format | same (shared `Displayable`) | same |

Diff of the syscall-name sequences (DBT vs ptrace-minus-execve): exactly two
lines differ —
```
ptrace-only: prlimit64   (glibc startup resource probe)
DBT-only:    ioctl        (glibc tty/stdout probe)
```
...
1. **`execve` invisibility (DBT):** DynamoRIO begins instrumenting *after* the
   process is already exec'd, so the DBT tool never sees the initial `execve`
   that launched the guest. ptrace catches it via `PTRACE_TRACEME`. This is
   inherent to in-process DBT, not a gap.
2. **errno formatting:** DBT prints `= -1 (EINVAL)` (name-decoded); the ptrace
...
3. **struct-result decoding:** ptrace re-reads output structs post-return (e.g.
   `fstat(...) -> {st_mode=...}`); the DBT tool shows the argument pointer. A
...
- `/bin/true`: DBT StraceTool traced 31 syscalls, clean `exit_group(0)`.
- `SyscallCounterTool` on `/bin/echo`: histogram `112 calls, 18 distinct`
  (`write`×1, `mmap`×21, `openat`×30, …).

## Conclusion

The DBT Guest/Tool interface hosts a real, observational `reverie::Tool` with
...
`execve` invisibility of in-process DBT. This confirms the interface is sound
